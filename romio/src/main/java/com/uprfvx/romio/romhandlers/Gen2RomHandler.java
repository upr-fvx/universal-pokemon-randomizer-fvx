...
import com.uprfvx.romio.graphics.palettes.Gen2SpritePaletteID;
...
//        new Gen2EncounterAreaTagger().tag(encounterAreas, getROMType(), false);
    ...
    private void readFishingEncounters(List<EncounterArea> encounterAreas) {
        ...
        int timeGroupsOffset = romEntry.getIntValue("FishingWildsTimeGroupsOffset");
        for (int i = 0; i < 44; i++) {
    ...
    private void writeNormalFishingEncounters(EncounterArea area, int offset) {
        checkNormalFishingEncountersSize(area, offset);
    ...
    private void checkNormalFishingEncountersSize(EncounterArea area, int offset) {
        int encCount = 0;
        int probability;
        do {
            probability = rom[offset] & 0xFF;
