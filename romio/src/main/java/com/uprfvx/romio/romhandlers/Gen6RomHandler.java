...
import com.uprfvx.romio.ctr.AMX;
...
import com.uprfvx.romio.romhandlers.romentries.Gen6RomEntry;
...
public class Gen6RomHandler extends Abstract3DSRomHandler {
    ...
    private void loadBasicPokeStats(Species pkmn, byte[] stats, Map<Integer,FormeInfo> altFormes) {
        ...
        // Held Items?
        Item item1 = items.get(IOFunctions.read2ByteInt(stats, Gen6Constants.bsCommonHeldItemOffset));
        Item item2 = items.get(IOFunctions.read2ByteInt(stats, Gen6Constants.bsRareHeldItemOffset));
        ...
        if (formeCount > 1) {
            if (!altFormes.containsKey(pkmn.getNumber())) {
                int firstFormeOffset = IOFunctions.read2ByteInt(stats, Gen6Constants.bsFormeOffset);
    ...
    private void populateMegaEvolutions() {
        ...
        try {
            ...
            for (int i = 1; i <= Gen6Constants.pokemonCount; i++) {
                Species pk = pokes[i];
                byte[] megaEvoEntry = megaEvoGARC.getFile(i);
        ...
        try {
            ...
            for (int i = 1; i <= moveCount; i++) {
                ...
                if (romEntry.getRomType() == Gen6Constants.Type_ORAS) {
                    moveData = Mini.UnpackMini(moveGarc.getFile(0), "WD")[i];
                } else {
                    moveData = moveGarc.getFile(i);
                }
                moves[i] = new Move();
    ...
    public void saveSpeciesStats() {
        ...
        int fileIndex = Gen6Constants.pokemonCount + Gen6Constants.getFormeCount(romEntry.getRomType()) + 1;

        try {
            byte[] duplicateData = pokeGarc.getFile(fileIndex);
            for (int i = 1; i <= Gen6Constants.pokemonCount + Gen6Constants.getFormeCount(romEntry.getRomType()); i++) {
    ...
    private void saveBasicPokeStats(Species pkmn, byte[] stats) {
        ...
        stats[Gen6Constants.bsHPOffset] = (byte) bs.getHp();
        stats[Gen6Constants.bsAttackOffset] = (byte) bs.getAttack();
        stats[Gen6Constants.bsDefenseOffset] = (byte) bs.getDefense();
        stats[Gen6Constants.bsSpeedOffset] = (byte) bs.getSpeed();
        stats[Gen6Constants.bsSpAtkOffset] = (byte) bs.getSpatk();
        stats[Gen6Constants.bsSpDefOffset] = (byte) bs.getSpdef();
        ...
        if (pkmn.getGuaranteedHeldItem() != null) {
            IOFunctions.write2ByteInt(stats, Gen6Constants.bsCommonHeldItemOffset, pkmn.getGuaranteedHeldItem().getId());
            IOFunctions.write2ByteInt(stats, Gen6Constants.bsRareHeldItemOffset, pkmn.getGuaranteedHeldItem().getId());
        } else {
            IOFunctions.write2ByteInt(stats, Gen6Constants.bsCommonHeldItemOffset,
                    pkmn.getCommonHeldItem() == null ? 0 : pkmn.getCommonHeldItem().getId());
            IOFunctions.write2ByteInt(stats, Gen6Constants.bsRareHeldItemOffset,
    ...
    private void writeShedinjaEvolution() throws IOException {
        ...
        extraEvolution.getEvolutionsTo().removeFirst();
    ...
    private void recreateFeebasBeautyEvolution() {
        ...
        Evolution prismScaleEvo = feebas.getEvolutionsFrom().getFirst();
    ...
    public void saveMoves() {
        try {
            ...
            if (romEntry.getRomType() == Gen6Constants.Type_ORAS) {
                miniArchive = Mini.UnpackMini(moveGarc.getFile(0), "WD");
            }
            for (int i = 1; i <= moveCount; i++) {
                byte[] data;
                if (romEntry.getRomType() == Gen6Constants.Type_ORAS) {
    ...
    public List<Species> getStarters() {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                int species = IOFunctions.read2ByteInt(staticCRO,offset+i*size);
                Species pk = pokes[species];
                int forme = staticCRO[offset+i*size + 4];
                // TODO: figure out how to best deal with the special forme ids like 30 and 31
                //  Is there a way of keeping them instead of just flattening?
    ...
    public boolean setStarters(List<Species> newStarters) {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                SpeciesHolder sh = newStatic.getSpeciesHolder();
                sh.setAltFormeAllowed();
                sh.setFormeNumber(starter.getFormeNumber());

                writeWord(staticCRO,offset+i*size, newStatic.getSpecies().getBaseNumber());
                staticCRO[offset+i*size + 4] = (byte) newStatic.getSpeciesHolder().getFormeNumber();
//                staticCRO[offset+i*size + 5] = (byte)newStatic.level;
                writeWord(displayCRO,displayOffset+displayIndex*0x54, newStatic.getSpecies().getBaseNumber());
                displayCRO[displayOffset+displayIndex*0x54+2] = (byte) newStatic.getSpeciesHolder().getFormeNumber();
                if (displayIndex < 3) {
    ...
    public List<Item> getStarterHeldItems() {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                if (!starterIndices.contains(i)) continue;
                int id = IOFunctions.readFullInt(staticCRO,offset+i*size + 12);
                if (id < 0) {
                    id = 0;
                }
    ...
    public void setStarterHeldItems(List<Item> items) {
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (item == null) {
                    IOFunctions.writeFullInt(staticCRO, offset + i * size + 12, -1);
                } else {
                    IOFunctions.writeFullInt(staticCRO, offset + i * size + 12, item.getId());
                }
            }
            writeFile(romEntry.getFile("StaticPokemon"),staticCRO);
        } catch (IOException e) {
    ...
    private List<EncounterArea> getEncountersXY() throws IOException {
        ...
        for (int i = 0; i < encounterGarc.size() - 1; i++) {
            byte[] b = encounterGarc.getFile(i);
            if (!wildMapNames.containsKey(i)) {
                wildMapNames.put(i, "? Unknown ?");
            }
            String mapName = wildMapNames.get(i);
            int offset = IOFunctions.readFullInt(b, 0x10) + 0x10;
    ...
    private List<EncounterArea> getEncountersORAS() throws IOException {
        GARCArchive encounterGarc = readGARC(romEntry.getFile("WildPokemon"), false);
        List<EncounterArea> encounterAreas = new ArrayList<>();
        for (int i = 0; i < encounterGarc.size() - 2; i++) {
            byte[] b = encounterGarc.getFile(i);
            if (!wildMapNames.containsKey(i)) {
                wildMapNames.put(i, "? Unknown ?");
            }
            String mapName = wildMapNames.get(i);
            int offset = IOFunctions.readFullInt(b, 0x10) + 0xE;
            int offset2 = IOFunctions.readFullInt(b, 0x14);
    ...
    private void setEncountersORAS(List<EncounterArea> encounterAreas) throws IOException {
        ...
        GARCArchive encounterGarc = readGARC(encountersFile, false);
        Iterator<EncounterArea> areaIterator = encounterAreas.iterator();
        byte[] decStorage = encounterGarc.getFile(encounterGarc.size() - 1);
        for (int i = 0; i < encounterGarc.size() - 2; i++) {
            byte[] b = encounterGarc.getFile(i);
            int offset = IOFunctions.readFullInt(b, 0x10) + 0xE;
            int offset2 = IOFunctions.readFullInt(b, 0x14);
            ...
            int decStorageOffset = IOFunctions.readFullInt(decStorage, (i + 1) * 4) + 0xE;
    ...
    private void updatePokedexAreaDataFromEncounterArea(EncounterArea area, byte[] pokedexAreaData, int areaIndex, int encounterType) {
        for (Encounter enc : area) {
            Species pk = enc.getSpecies().getBaseForme();
            ...
            int value = IOFunctions.readFullInt(pokedexAreaData, offset);
            value |= encounterType;
            IOFunctions.writeFullInt(pokedexAreaData, offset, value);
        }
    }
    ...
    private void loadWildMapNames() {
        try {
            ...
            byte[] zoneData = encounterGarc.getFile(zoneDataOffset);
        ...
        try {
            ...
            for (int i = 1; i < trainernum; i++) {
                ...
                for (int poke = 0; poke < numPokes; poke++) {
                    ...
                    writeWord(trpoke, pokeOffs + 6, tp.getSpeciesHolder().getFormeNumber());
                    pokeOffs += 8;
                    if (tr.pokemonHaveItems()) {
...
//                }
                movedata = movesLearnt.getFile(i);
                int moveDataLoc = 0;
...
//                }
    ...
    public List<StaticEncounter> getStaticPokemon() {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                short heldItem = (short)IOFunctions.read2ByteInt(staticCRO,offset+i*size + 4);
                if (heldItem < 0) {
                    heldItem = 0;
                }
            ...
            for (int i = 0; i < count; i++) {
                ...
                sh.setFormeNumber(forme);
                se.setLevel(staticCRO[offset+i*size + 5]);
                int heldItem = IOFunctions.readFullInt(staticCRO,offset+i*size + 12);
                if (heldItem < 0) {
                    heldItem = 0;
                }
                se.setHeldItem(items.get(heldItem));
                if (romEntry.getRomType() == Gen6Constants.Type_ORAS) {
                    int metLocation = IOFunctions.read2ByteInt(staticCRO, offset + i * size + 18);
    ...
    private StaticEncounter readTrashCanEncounter(int offset) {
        int species = IOFunctions.readFullInt(code, offset);
        int forme = IOFunctions.readFullInt(code, offset + 4);
        int level = IOFunctions.readFullInt(code, offset + 8);
        Species pk = pokes[species];
        if (forme == 30 || forme == 31) {
            forme = 0;
        }
    ...
    public boolean setStaticPokemon(List<StaticEncounter> staticPokemon) {
        ...
        try {
            ...
            for (int i = 0; i < staticCount; i++) {
                ...
                staticCRO[offset+i*size + 2] = (byte) se.getSpeciesHolder().getFormeNumber();
            ...
            for (int i = 0; i < giftCount; i++) {
                ...
                StaticEncounter se = staticIter.next();
                writeWord(staticCRO,offset+i*size, se.getSpecies().getBaseNumber());
                staticCRO[offset+i*size + 4] = (byte) se.getSpeciesHolder().getFormeNumber();
                staticCRO[offset+i*size + 5] = (byte) se.getLevel();
                if (se.getHeldItem() == null) {
                    IOFunctions.writeFullInt(staticCRO,offset+i*size + 12,-1);
                } else {
                    IOFunctions.writeFullInt(staticCRO,offset+i*size + 12, se.getHeldItem().getId());
                }
            }
            writeFile(romEntry.getFile("StaticPokemon"),staticCRO);
            ...
            if (romEntry.getRomType() == Gen6Constants.Type_XY) {
                ...
                if (offset > 0) {
                    ...
                    while (currentCount != Gen6Constants.xyTrashCanEncounterCount) {
                        ...
                        IOFunctions.writeFullInt(code, offset + 4, se.getSpeciesHolder().getFormeNumber());
                        ...
                        for (int i = 0; i < se.getLinkedEncounters().size(); i++) {
                            ...
                            IOFunctions.writeFullInt(code, offset + 4, linkedEncounter.getSpeciesHolder().getFormeNumber());
            ...
            if (romEntry.getRomType() == Gen6Constants.Type_XY) {
                ...
                fixBoxLegendariesXY(boxLegendaryEncounter.getSpecies().getBaseNumber());
                ...
                fixRayquazaORAS(rayquazaEncounter.getSpecies().getBaseNumber());
            }
    ...
    private void unlinkStaticEncounters(List<StaticEncounter> statics) {
        ...
        for (Integer offsetToInsert : offsetsToInsert) {
            // shouldn't matter what's added here so we add bulbasaur
            statics.add(offsetToInsert, new StaticEncounter(pokes[SpeciesIDs.bulbasaur]));
        }
        for (ThreeDSLinkedEncounter le : romEntry.getLinkedEncounters()) {
            StaticEncounter baseEncounter = statics.get(le.getBase());
            statics.set(le.getLinked(), baseEncounter.getLinkedEncounters().getFirst());
        }
    }
    ...
    private void fixBoxLegendariesXY(int boxLegendarySpecies) throws IOException {
        ...
        for (int offset : boxLegendaryScriptOffsets) {
            IOFunctions.write2ByteInt(data, offset, boxLegendarySpecies);
        }
    ...
    private void fixRayquazaORAS(int rayquazaEncounterSpecies) throws IOException {
        ...
        AMX rayquazaAMX = new AMX(scriptGarc.getFile(rayquazaScriptFile));
        byte[] data = rayquazaAMX.decData;
        for (int i = 0; i < Gen6Constants.rayquazaScriptOffsetsORAS.length; i++) {
            IOFunctions.write2ByteInt(data, Gen6Constants.rayquazaScriptOffsetsORAS[i], rayquazaEncounterSpecies);
        }
    ...
    public void giveNationalDexAtStart() {
        ...
        if (romEntry.getRomType() == Gen6Constants.Type_XY) {
            ...
            if (offset > 0) {
                ...
                IOFunctions.writeFullInt(code, offset, 0);
            ...
            if (offset > 0) {
                ...
                IOFunctions.writeFullIntBigEndian(code, offset + 32, 0x0700A0E1);
                IOFunctions.writeFullIntBigEndian(code, offset + 36, 0x000050E3);
                IOFunctions.writeFullIntBigEndian(code, offset + 40, 0x0100A003);
                IOFunctions.writeFullIntBigEndian(code, offset + 44, 0xF09FBDE8);
                ...
                // wrote above.
                IOFunctions.writeFullIntBigEndian(code, offset + 208, 0xD2FFFFEA);
            }
        }
    }
    ...
    public void setMoveTutorMoves(List<Integer> moves) {
        ...
        if (mtOffset > 0) {
            ...
            for (int i = 0; i < Gen6Constants.tutorMoveCount; i++) {
                IOFunctions.write2ByteInt(code,mtOffset + i*8, moves.get(i));
            }
        }
        ...
        if (mtOffset > 0) {
            for (int move: moves) {
                int val = IOFunctions.read2ByteInt(code,mtOffset);
    ...
    public List<Integer> getFieldItemIds() {
        ...
        try {
            ...
            GARCArchive scriptGarc = readGARC(romEntry.getFile("Scripts"),true);
            AMX normalItemAMX = new AMX(scriptGarc.getFile(normalItemsFile));
            byte[] data = normalItemAMX.decData;
            for (int i = normalItemsOffset; i < data.length; i += 12) {
                int item = IOFunctions.read2ByteInt(data,i);
                fieldItems.add(item);
            }
            ...
            if (romEntry.getRomType() == Gen6Constants.Type_XY) {
                ...
                AMX hiddenItemAMX = new AMX(scriptGarc.getFile(hiddenItemsFile));
                ...
                if (offsHidden > 0) {
                    ...
                    for (int i = 0; i < Gen6Constants.hiddenItemCountORAS; i++) {
                        int item = IOFunctions.read2ByteInt(code, offsHidden + (i * 0xE) + 2);
                        fieldItems.add(item);
                    }
                }
            }
    ...
    private List<Integer> getFieldMegaStonesORAS(GARCArchive scriptGarc) throws IOException {
        ...
        for (int i = 0; i < Gen6Constants.megastoneTableLengthORAS; i++) {
            ...
            int item = IOFunctions.read2ByteInt(megaStoneItemEvent.decData, offset);
    ...
    public void setFieldItemIds(List<Integer> items) {
        try {
            ...
            for (int i = normalItemsOffset; i < data.length; i += 12) {
                ...
                IOFunctions.write2ByteInt(data,i,item);
            }
            ...
            if (romEntry.getRomType() == Gen6Constants.Type_XY) {
                ...
                if (offsHidden > 0) {
                    ...
                    for (int i = 0; i < Gen6Constants.hiddenItemCountORAS; i++) {
                        int item = iterItems.next();
                        IOFunctions.write2ByteInt(code,offsHidden + (i * 0xE) + 2, item);
                    }
                }
            }
            ...
            if (romEntry.getRomType() == Gen6Constants.Type_ORAS) {
                ...
                for (int i = 0; i < Gen6Constants.megastoneTableLengthORAS; i++) {
                    ...
                    if (megaStoneMap.containsKey(oldItem)) {
                        ...
                        IOFunctions.write2ByteInt(megaStoneItemEvent.decData, offset, replacementItem);
                    } else {
                        IOFunctions.write2ByteInt(megaStoneItemEvent.decData, offset, newItem);
    ...
    public void setInGameTrades(List<InGameTrade> trades) {
        ...
        if (offset > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                IOFunctions.write2ByteInt(code,offset, trade.getGivenSpecies().getNumber());
                for (int iv = 0; iv < 6; iv++) {
                    code[offset + 5 + iv] = (byte) trade.getIVs()[iv];
                }
                IOFunctions.write2ByteInt(code,offset + 0xE, trade.getOtId());
                IOFunctions.write2ByteInt(code,offset + 0x10, trade.getHeldItem() == null ? 0 : trade.getHeldItem().getId());
                tradeStrings.set(textOffset + count + i, trade.getOtName());
                IOFunctions.write2ByteInt(code,offset + 0x20,
    ...
    public List<Shop> getShops() {
        ...
        for (int i = 0; i < shopCount; i++) {
            ...
            for (int j = 0; j < shopItemSizes[i]; j++) {
                shopItems.add(items.get(IOFunctions.read2ByteInt(code, offset)));
                offset += 2;
            }
            Shop shop = new Shop();
    ...
    public void setShops(List<Shop> shops) {
        ...
        for (int i = 0; i < shopCount; i++) {
            ...
            for (int j = 0; j < shopItemSizes[i]; j++) {
                ...
                IOFunctions.write2ByteInt(code, offset, item.getId());
                offset += 2;
            }
        }
    }
    ...
    public List<Integer> getShopPrices() {
        ...
        try {
            ...
            for (int i = 1; i < itemPriceGarc.size(); i++) {
                prices.add(readWord(itemPriceGarc.getFile(i), 0) * 10);
            }
    ...
    public void setShopPrices(List<Integer> prices) {
        ...
        try {
            GARCArchive itemPriceGarc = this.readGARC(romEntry.getFile("ItemData"), true);
            for (int i = 1; i < itemPriceGarc.size(); i++) {
                writeWord(itemPriceGarc.getFile(i), 0, prices.get(i) / 10);
            }
            writeGARC(romEntry.getFile("ItemData"), itemPriceGarc);
        } catch (IOException e) {
            throw new RomIOException(e);
    ...
    public List<PickupItem> getPickupItems() {
        ...
        if (pickupItemsTableOffset > 0) {
            for (int i = 0; i < Gen6Constants.numberOfPickupItems; i++) {
                ...
                int id = IOFunctions.read2ByteInt(code, itemOffset);
    ...
    public void setPickupItems(List<PickupItem> pickupItems) {
        if (pickupItemsTableOffset > 0) {
            for (int i = 0; i < Gen6Constants.numberOfPickupItems; i++) {
                ...
                IOFunctions.write2ByteInt(code, itemOffset, id);
            }
        }
    }
    ...
    public BufferedImage getPokemonIcon(int iconIndex, GARCArchive pokeGraphicsGARC,
        ...
        byte[] icon;
        try {
            icon = pokeGraphicsGARC.getFile(iconIndex);
        } catch (IOException e) {
            throw new RomIOException(e);
        }
